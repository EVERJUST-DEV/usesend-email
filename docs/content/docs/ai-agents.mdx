...
<Callout type="info">
The fastest way to give an agent email is the [useSend MCP server](/mcp) — it exposes send / status / domain tools directly to the model, so you write no HTTP glue. Reach for the raw REST patterns below when you are building your own tool wrappers or working outside an MCP host.
</Callout>
...
<Callout type="warn">
SES is currently in **sandbox**. Until AWS grants production access, sends only succeed to verified addresses or the SES mailbox simulator (`success@simulator.amazonses.com`, `bounce@…`, `complaint@…`). An agent testing end-to-end should target `success@simulator.amazonses.com` — it accepts and "delivers" without a real inbox. See [Sending email](/sending-email) for the full sandbox rules.
</Callout>
...
Terminal-failure statuses (`BOUNCED`, `COMPLAINED`, `REJECTED`, `FAILED`, `SUPPRESSED`) mean the address will not accept mail — an agent should stop retrying that recipient rather than loop. Full status list is in the [API reference](/api).
...
See [Webhooks](/webhooks) for the full event catalog, signature scheme, and payload shapes.
...
<Cards>
  <Card title="MCP server" href="/mcp">Ready-made send / status / domain tools — the least-glue path for an agent.</Card>
  <Card title="API reference" href="/api">Every endpoint, field, error code, and the full status enum.</Card>
  <Card title="Sending email" href="/sending-email">Send options, templates, attachments, scheduling, and sandbox rules.</Card>
  <Card title="Webhooks" href="/webhooks">Event types, HMAC signature verification, and retry behavior.</Card>
</Cards>
