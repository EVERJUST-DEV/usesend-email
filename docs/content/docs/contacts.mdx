...
Contact books are useSend's audience feature: named lists of contacts that [campaigns](/docs/campaigns) send to. A contact book holds many contacts, each with an email, optional name, subscription state, and custom `properties`. Double opt-in is configurable per book.

<Callout type="info">
Contacts are for **marketing/broadcast** sending only. Transactional sends via [`POST /v1/emails`](/docs/emails) take a raw `to` address and do **not** require a contact or contact book. Reach for this API only when you are building lists for [campaigns](/docs/campaigns).
</Callout>
...
<Cards>
  <Card title="Campaigns" href="/docs/campaigns">Send a broadcast to a contact book.</Card>
  <Card title="Sending email" href="/docs/emails">Transactional sends — no contacts required.</Card>
  <Card title="Webhooks" href="/docs/webhooks">Subscribe to contact.created / updated / deleted events.</Card>
</Cards>
