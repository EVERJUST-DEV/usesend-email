...
<Callout type="info">
The EverJust Team already has a verified sending domain, **`send.everjust.app`** (`status: SUCCESS`). You only need these endpoints when adding a new domain. To send mail from an existing domain, see [Send an email](/sending-email).
</Callout>
...
<Cards>
  <Card title="Send an email" href="/sending-email">
