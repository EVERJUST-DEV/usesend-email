...
<Callout type="info">
**Not supported by the API:** `tags`, and on attachments `path` / `contentType`. Attachment bodies must be inlined as base64 `content`. Template CRUD, suppressions, webhooks, and API-key creation are dashboard-only — see [Webhooks](/docs/webhooks) and [Domains](/docs/domains).
</Callout>
...
To list sent email, use `GET /v1/emails` with `page`, `limit`, `startDate`, `endDate`, and `domainId` query parameters. For push-based delivery of these transitions, configure [Webhooks](/docs/webhooks).
...
<Cards>
  <Card title="Domains" href="/docs/domains">Verify and inspect sending domains.</Card>
  <Card title="Webhooks" href="/docs/webhooks">Receive delivery, bounce, open, and click events.</Card>
</Cards>
