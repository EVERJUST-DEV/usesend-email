...
It is a thin wrapper over the [REST API](/docs/api-reference): every tool maps to one endpoint, authenticates with a `us_` Bearer key, and returns the raw JSON envelope. Anything the API can do, the agent can do.
...
Every tool corresponds to one REST endpoint. Fields, enums, and response shapes match the [API reference](/docs/api-reference) exactly — the server neither adds nor renames fields.
...
<Cards>
  <Card title="Sending email" href="/docs/emails">The send payload, scheduling, sandbox rules, and status enum.</Card>
  <Card title="API reference" href="/docs/api-reference">Every endpoint, field, and error code the tools wrap.</Card>
  <Card title="Domains" href="/docs/domains">Provisioning and verifying sending domains.</Card>
  <Card title="Webhooks" href="/docs/webhooks">Delivery events and signature verification (dashboard-configured).</Card>
</Cards>
